...
- An IMU, I will use an MPU6050. Any IMU will do the job but you will need to adapt the I2C communication part.
...
This allows the use of both advantages:
...
Accelerometer for long-term correction of our orientation and gyroscope for short-term correction.
...
One of the most popular one is the [Kalman Filter](https://en.wikipedia.org/wiki/Kalman_filter). This filter has two steps:
 
### Prediction Step
The filter is able to predict the current state based on:
- The lastest estimation
- The matrix model that will be described below.
The prediction step is used when we have a sensor that has a frequency to low. The algorithm is still able to predict steps if we want a faster loop than the sensor is able to give us.
...
### Correction Step
This step is called every time new sensor data are available. With the lastest estimation, the confidence in it's prediction (stored as 'noise' matrices) it will 'correct' (denoise) the new sensor data.
...
<p align="center">
<img src="imgs/rcc_clock_config.png"/>
</p>
...
<p align="center">
<img src="imgs/clock_config.png"/>
</p>

- Finally in Pinout and Configuration
- I2C on yours MPU6050 pins (mine is on I2C1 with pins PB8; PB9)
- UART for telemetry output (mine is on UART6 with pins PA11; PA12)
- In System Core / SYS enable Debug: Serial Wire if you use an ST-Link for debugging
<p align="center">
<img src="imgs/debug_config.png"/>
</p>

- For the Kalman Filter we need a proper delta time. Even though it is possible to have a non constant delta time, we will use a timer to get a known and stable delta time. So Enable any **General purpose timer**, like TIM2 and enable the global interrupt associated to this one.
<p align="center">
<img src="imgs/tim_it_config.png"/>
</p>


- We will refresh the sensor at `100Hz` to do this we need to setup the timer.
- Use a `Prescaler` of `HCLK - 1`. HCLK id the value you have set before in Clock Configuration. (In that case you will always end up with a 1 MHz Timer)
- Use a `Counter Period` of `10000 - 1`. 1MHz divided by `10'000` is `100Hz`
<p align="center">
<img src="imgs/tim_config.png"/>
</p>

**You should end up with something like this:**
<p align="center">
<img src="imgs/stm_final_config.png"/>
</p>
...
The first step is to set up the toolchain.
...
I will go for the latter but here are the two variants (You need to be in the working directory of the project):
...
The github version hasn't been maintained for 6 years now. Since it is more popular to have a github account, I will use the github one. It is way enough for the little math we will use.
(Prefer using the gitlab version with real project thought) 
...
```
 ...
 - Since I going to use Teleplot to draw graph, I need to print floating point. Printing floating point is not supported by default on arm, so add `-u _printf_float` to `LDFLAGS`.
...
- Last one: to be able to flash our board easely, you can add the `flash` rule (You will need to check the upload address of your MCU configuration. It should be ok if you are using the same as me):
```Makefile
flash: all
...
In order to get printf working, we have to define `_write`.
Using the UART defined in CubeMX (for my case is was `huart6`):
```cpp
...
#### Receiving Data
As described earlier, we are going to read the sensor at 100Hz using a timer interrupt. So first thing first, we need to launch the interrupt callback. (I have used TIM2)
```c
// Launch the timer interrupt callback
HAL_TIM_Base_Start_IT(&htim2);
```

Each time our timer overflow, a callback is called, so let's define this function
```c
...
The MPU always sends data as a 16 bits signed integer. To do the conversion we will use two defines:
```cpp
...
To get all data at the same time we need to use an struct according to the data sent:
```cpp
struct MPU6050_data
{
...
The MPU is able to return the temperature, because gyroscope and accelerometer are sensitive to temperature.
...
I have save my Teleplot layout, you can import it from [.vscode/teleplot_layout.json](.vscode/teleplot_layout.json)

<p align="center">
<img src="imgs/gyro_raw.png"/>
</p>

<p align="center">
<img src="imgs/accel_raw.png"/>
</p>
...
So let explain breifly Kalman Filter equations. First with it's components:
 ...
 - `Q` and `R`: Are both Matrix2*2, There are const. They need to be tuned correctly to select whether the sytem is more "confident" to the prediction or more to the actual measurment. `Q` is how "not confident"(noise) we are on our prediction, and `R` store how "not confident"(noise) we are on our measurment.
...
// Change this Matrix regarding the data you want to use, and according to your device.
...
<p align="center">
<img src="imgs/kf_roll.png"/>
</p>

<p align="center">
<img src="imgs/kf_roll_rate.png"/>
</p>
...
<p align="center">
<img src="imgs/roll_kf_vs_gyro.png"/>
</p>
